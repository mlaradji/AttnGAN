...
def parse_args():
    parser = argparse.ArgumentParser(description='Download data, preprocessed metadata, pretrained DAMSencoders and fully trained models for either the birds or the COCO dataset.')
    ...
    parser.add_argument('--raw', action="store_const", default=False, const=True, 
                        help='Download the raw images for the dataset.')
    parser.add_argument('--metadata', action="store_const", default=False, const=True, 
                        help='Download preprocessed metadata.')
    parser.add_argument('--damsme', action="store_const", default=False, const=True, 
                        help='Download pretrained DAMSMencoders.')
    parser.add_argument('--models', action="store_const", default=False, const=True, 
                        help='Download fully-trained models.')
    parser.add_argument('--overwrite', action="store_const", default=False, const=True, 
...
def main(args):
    ...
    downloads = {
        "birds": [
            ...
            {"source": "gdrive", "id": "1O_LtUP9sch09QH3s_EBAgLEctBQ5JBSJ", "path": folder["data"] / "birds.zip", "type": "metadata"},
            # DAMSMencoders
            {"source": "gdrive", "id": "1GNUKjVeyWYBJ8hEU-yrfYQpDOkxEyP3V", "path": folder["damsme"] / "bird.zip", "type": "damsme"},
            # Models
            {
            ...
            # Image data.
            {"source": "url", "url": "http://www.vision.caltech.edu/visipedia-data/CUB-200-2011/CUB_200_2011.tgz", "path": folder["data"] / "CUB_200_2011.tgz", "extract_to": folder["data"]/"birds", "type": "raw"},
        ],
        "coco": [
            # Preprocessed metadata
            {"source": "gdrive", "id": "1rSnbIGNDGZeHlsUlLdahj0RJ9oo6lgH9", "path": folder["data"] / "coco.zip", "type": "metadata"},
            # DAMSMencoders
            {"source": "gdrive", "id": "1zIrXCE9F6yfbEJIbNP5-YrEe2pZcPSGJ", "path": folder["damsme"] / "coco.zip", "type": "damsme"},
            # Models
            {
    ...
    allowed_types = conditional_set(*(("metadata", args.metadata), ("damsme", args.damsme), ("raw", args.raw), ("models", args.models)))
    print(allowed_types)
    for file in filter(lambda d: d["type"] in allowed_types, downloads[args.dataset]):

        if not(args.overwrite) and file["path"].exists():
            print("Skipped downloading '%s' as it already exists. Use the '--overwrite' flag to force redownload." % file["path"])
            continue
